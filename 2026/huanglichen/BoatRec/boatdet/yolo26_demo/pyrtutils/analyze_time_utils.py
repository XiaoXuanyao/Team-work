...
# openpyxl 仅用于时序/精度统计导出 Excel，推理本身不需要；板上无该库时允许跳过
try:
    from openpyxl.styles import Border, Side, Alignment, PatternFill
    from openpyxl.utils import get_column_letter
    HAS_OPENPYXL = True
except ImportError:
    Border = Side = Alignment = PatternFill = None
...
def calculate_hardop_class_time(writer,df,sheet_name,startcol=12,startrow=0):
    ...
    # 2. 如果存在 is_io_process 列，则执行新的拆分合并逻辑
        ...
        # 2.3. 合并统计结果
        df_class = pd.concat([backbone_counts, backbone_times, io_counts], axis=1)
        
        # 2.4. 填充NaN值并转换数据类型
        df_class.fillna(0, inplace=True)
        df_class['backbone_op_num'] = df_class['backbone_op_num'].astype(int)
        df_class['io_op_num'] = df_class['io_op_num'].astype(int)

        # 2.5. 剔除 memcpy_time
        df_class["total_time"] = df_class["total_time"] - df_class["memcpy_time"]
        df_class.drop(["memcpy_time"],axis=1,inplace=True)

        # 2.6. 计算并添加 'total_hardop' 总结行
        total_row = df_class.sum()
        total_row.name = 'total_hardop'
        df_class = df_class.append(total_row)

        # 2.7. 调整最终列顺序
        df_class = df_class[['backbone_op_num', 'total_time', 'hard_time', 'other_time', 'io_op_num']]
    
    # 3. 统一的收尾工作：四舍五入并写入Excel
    df_class = df_class.round(4)
    title_row = pd.DataFrame(index=['各类HardOP算子的耗时细则：'])
    title_row.to_excel(writer, sheet_name=sheet_name, startcol=startcol, startrow=startrow)
    df_class.to_excel(writer, sheet_name=sheet_name, startcol=startcol,startrow=startrow+2)
...
def calculate_detailtime(writer,df,sheet_name,startcol=12,startrow=25):
    ...
    # io算子耗时
    if 'is_io_process' in df.columns:
        io_time = df[df['is_io_process'] == True]['total_time'].sum()

        # --- 新增修改 1: 从 icore_time 中减去 io_time ---
        icore_time = icore_time - io_time

        # 呈现给用户的时间
        # row_names = ["数据传入时间", "icore(npu)时间","数据传出时间","Icraft-CPU算子耗时","后处理耗时(自行填写)",'io算子时间']
        # column_data = [icore_in_time, backbone_time, icore_out_time, cpu_time, post_time, io_time]
        # df_detailtime = pd.DataFrame(column_data, index=row_names,columns=['time(ms)'])
        # df_detailtime['Device'] =[imk_list,other_customop_list,post_list,"null","null","npu"]
        row_names = ["icore(npu)时间","Icraft-CPU算子耗时","后处理耗时(自行填写)",'io算子时间']
        column_data = [icore_time, cpu_time, post_time, io_time]
        df_detailtime = pd.DataFrame(column_data, index=row_names,columns=['time(ms)'])
        df_detailtime['Device'] =[other_customop_list,"null","null","npu"]
    else:
        # 呈现给用户的时间
    ...
    # 延时
    delay_time = '=SUM(Sheet1!B4:B9)'
    # 吞吐率
    fps = '=1000/MAX(Sheet1!B4:B9)'
    ...
    bottleneck_formula = '=INDEX(A4:A9,MATCH(MAX(B4:B9),B4:B9,0))' if 'is_io_process' in df.columns else '=INDEX(A4:A8,MATCH(MAX(B4:B8),B4:B8,0))'
...
def analyze_time(filepath,res_root="./res/",speedMode=False):
    """
    输入:
    ...
    # 确保结果目录存在。如果目录已存在，则清理旧的分析文件（_res.xlsx 和 .png），
    # 以避免与新生成的文件混淆。原始的输入xlsx文件将被保留。
    if os.path.exists(respath):
        print(f"结果目录 {respath} 已存在，正在清理旧的分析文件...")
        for item in os.listdir(respath):
            # 删除上一次分析生成的xlsx和所有png图片
            if item == f"{filename}_res.xlsx" or item.endswith('.png'):
                try:
                    os.remove(os.path.join(respath, item))
                except OSError as e:
                    print(f"删除文件 {item} 时出错: {e}")
    else:
        os.makedirs(respath)
        print(f"结果目录 {respath} 已创建。")
    ...
    if not HAS_OPENPYXL:
        raise RuntimeError("缺少 openpyxl，无法导出 Excel 统计（推理不受影响）")
