...
class Netinfo:
    ...
    def fpgaOPlist(self):
        ...
        for op in oplist:
            ...
            if "ImageMakeNode" in op.typeKey():
                ...
                self.ImageMake_ = op
            if "DetPostNode" in op.typeKey():#buyi detpost node name 
                self.DetPost_on = True
                self.DetPost_ = op  
            if "DetPostZGNode" in op.typeKey():#zg330 detpost node name 
                self.DetPost_on = True
                self.DetPost_ = op                
    ...
    def getNetInfo(self):
        ...
        compressFtmp = xir.Bool(self.network.getTag("compressFtmp")).__bool__()
        ...
        if self.DetPost_on:
            try:
                ...
                for i in self.DetPost_.inputs:
                    if(i.tensorType().element_dtype.getNormratio()):
                        # get scale for BY
                        self.o_scale.append(i.tensorType().element_dtype.getNormratio().data[0])
                    elif i.tensorType().element_dtype.getScale():
                        # get scale for ZG330
                        # getScale() 返回的是 Optional<QuantizedScaleArray>
                        scale = i.tensorType().element_dtype.getScale().front().origin_scale
                        self.o_scale.append(scale)
                    else:
                        scale = 1.0
                        self.o_scale.append(scale)
            except:
