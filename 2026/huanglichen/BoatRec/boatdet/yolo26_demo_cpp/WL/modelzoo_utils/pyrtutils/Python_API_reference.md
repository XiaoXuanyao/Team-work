...
- 参数:
  ...
  **outputs: List[icraft.xrt.Tensor]** – 需要转成对应ndarray的Tensor数据，类型为icraft.xrt.Tensor
...
- 说明:
  ...
  icraft.Tensor构造成ndarray的转换函数，传入一个List[icraft.xrt.Tensor]作为构造ndarray的指定pybuffer的数据，根据输入数据的value的datatype，通过tensortype中的数据存储类型将**输入数据的类型转换为对应的数据类型**，且根据value完成输入tensor的形状、layout、数据类型的定义。
...
- 参数:
  ...
  **session: xrt.Session** – 构造的 XRT 会话对象(icraft.xrt.Session)，用于执行模型推理

  **input_tensors** – 可以传入不同数目的输入数据，类型为xrt.Tensor，如果为多输入需要整理为列表[xrt.Tensor]

- 返回:

  list[xrt.Tensor]:从pl搬运到ps后的输出张量列表

- 说明:

  * (1)在指定的 xrt.Session 中执行模型推理<br>
  * (2)将所有输出张量手动从pl搬运到pl，便于后续处理
