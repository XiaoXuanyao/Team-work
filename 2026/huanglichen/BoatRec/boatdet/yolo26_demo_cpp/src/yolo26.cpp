...
int main(int argc, char* argv[])
{
	try
	{
		...
		// int bbox_info_channel = 64; // yolov10的参数配置
		...
		for (auto name : namevector) {
			...
			else if (outputs.size() == 1 && outputs[0].dtype()->shape.size() == 3) {
				// flat 输出 [1, candidates, 4+N_CLASS]
				post_detpost_flat(outputs, img, netinfo, conf, iou_thresh, MULTILABEL, fpga_nms, N_CLASS,
				ANCHORS, LABELS, show, save, resRoot, name, device, runBackend);
			}
			else{
