...
void post_detpost_hard(const std::vector<Tensor>& output_tensors, PicPre& img, NetInfo& netinfo,
    ...
    bool & show, bool & save , std::string &resRoot, std::string & name,icraft::xrt::Device device,const std::string& runBackend,std::vector<std::vector<float>> & _norm,std::vector<int> & real_out_channles,std::vector<float> & _stride,int bbox_info_channel) {
...
// 新版 flat 输出后处理: [1, 18, N] (N=8400/8500)
// 每个候选: 4 bbox(xywh grid相对坐标) + 14 cls(未sigmoid)
void post_detpost_flat(const std::vector<Tensor>& output_tensors, PicPre& img, NetInfo& netinfo,
    float conf, float iou_thresh, bool MULTILABEL, bool fpga_nms, int N_CLASS,
    std::vector<std::vector<std::vector<float>>>& ANCHORS, std::vector<std::string>& LABELS,
    ...
    for (auto &output_tensor : output_tensors) {
        auto host_tensor = output_tensor.to(HostDevice::MemRegion());
        auto shape = output_tensor.dtype()->shape;
        // shape: [1, candidates, channels]  (NHWC, **C layout)
        int num_candidates = shape[1];
        int num_channels = shape[2];
        auto tensor_data = (float*)host_tensor.data().cptr();

        for (int i = 0; i < num_candidates; i++) {
            float* ptr = tensor_data + i * num_channels;
            // bbox: ptr[0..3] (xywh grid-relative)
            float xc = ptr[0], yc = ptr[1], w = ptr[2], h = ptr[3];
            // cls: ptr[4..4+N_CLASS-1] (need sigmoid)
            if (!MULTILABEL) {
                int best_idx = -1;
                ...
                float prob = 1.0f / (1.0f + expf(-best_score));
                if (prob > conf) {
                    // 将 grid-relative bbox 转为原图坐标 (假设 stride=1 用 image resize 比例修正)
                    float x1 = (xc - w / 2.f);
                    float y1 = (yc - h / 2.f);
                    float x2 = (xc + w / 2.f);
                    float y2 = (yc + h / 2.f);
                    // 取输入图的短边缩放比例
                    float scale = (float)img.ori_img.cols / netinfo.i_cubic[0].w;
                    float x = x1 * scale;
                    float y = y1 * scale;
                    float bw = (x2 - x1) * scale;
                    float bh = (y2 - y1) * scale;
                    id_list.emplace_back(best_idx);
                    socre_list.emplace_back(prob);
                    box_list.emplace_back(cv::Rect2f(x, y, bw, bh));
                }
            } else {
                for (int j = 0; j < N_CLASS; j++) {
                    float prob = 1.0f / (1.0f + expf(-ptr[4 + j]));
                    if (prob > conf) {
                        float x1 = (xc - w / 2.f);
                        float y1 = (yc - h / 2.f);
                        float x2 = (xc + w / 2.f);
...
void post_detpost_soft(const std::vector<Tensor>& output_tensors, PicPre& img, std::vector<std::string>& LABELS,
    std::vector<std::vector<std::vector<float>>>& ANCHORS, NetInfo& netinfo, int N_CLASS, float conf, float iou_thresh,bool& fpga_nms, icraft::xrt::Device device, const std::string& runBackend,
        ...
        auto tensor_data_box = (float*)host_tensor_box.data().cptr();
         for (size_t h = 0; h < _H; h++) {
            for (size_t w = 0; w < _W; w++) {
                ...
                // auto boxPtr = tensor_data_box + h * _W * 64 + w * 64; //yolov10配置
                auto boxPtr = tensor_data_box + h * _W * 4 + w * 4; //yolo26配置
                if (!MULTILABEL) {
                    ...
                    if (realscore > conf) {
                        //getBbox
                        // std::vector<float> ltrb = dfl(boxPtr,1.0, 0, 64); //yolov10配置
                        std::vector<float> ltrb = {boxPtr[0],boxPtr[1], boxPtr[2], boxPtr[3]}; //yolo26配置
...
YoloPostResult post_detpost_plin(const std::vector<Tensor>& output_tensors, YoloPostResult& last_frame_result,NetInfo& netinfo,
    ...
    const std::string& runBackend,std::vector<std::vector<float>> & _norm,std::vector<int> & real_out_channles,std::vector<float> & _stride,int bbox_info_channel) {
