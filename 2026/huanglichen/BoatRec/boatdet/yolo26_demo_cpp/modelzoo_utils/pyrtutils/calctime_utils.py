...
def simple_analyze_time(runBackend,net_dict):
    ...
    if runBackend == "buyi":
        ...
        print(f"Hardop_HardTime(ms): {hardop_hardtime:.4f}")
        for i in range(len(customops["customop_name"])):
            if customops['customop_name'][i] == "ImageMake":
                continue
            print(f"Customop: {customops['customop_name'][i]}, HardTime(ms): {customops['hard_time'][i]:.4f}")
        print("******************************************************")
